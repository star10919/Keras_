...
### save_to_dir: 파일 저장 경로  // .next()[0]를 안쓰면 타입이 이터레이터라서 프린트 될때마다 실행되서 추가적으로 계속 생김 // .next()[0]를 쓰면 전체 한번만 실행되니까 딱 한번만 저장됨
...
x_augmented = train_datagen.flow(x_augmented, np.zeros(augment_size), batch_size=augment_size, shuffle=False,   # flow에서 출력 : iterator
