...
model.fit(x_train_noised, x_train, epochs=10, batch_size=512)
...
for i, ax in enumerate([ax1, ax2, ax3, ax4, ax5]):
    ax.imshow(x_test[random_images[i]].reshape(28, 28, 1)*255, cmap='gray')
    if i == 0:
...
for i, ax in enumerate([ax11, ax12, ax13, ax14, ax15]):
    ax.imshow(x_test_noised[random_images[i]].reshape(28, 28, 1)*255, cmap='gray')
    if i == 0:
...
for i, ax in enumerate([ax6, ax7, ax8, ax9, ax10]):
    ax.imshow(output[random_images[i]].reshape(28, 28, 1)*255, cmap='gray')
    if i == 0:
